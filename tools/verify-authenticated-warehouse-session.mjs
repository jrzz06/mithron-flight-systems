...
async function verifyRouteAccessWithBrowser(credentials) {
  ...
  try {
    await page.goto(`${baseUrl}/login?next=/warehouse/inventory`, { waitUntil: "networkidle" });
    await page.locator("input[type='email']").fill(credentials.email);
    await page.locator("input[type='password']").fill(credentials.password);
    await Promise.all([
      page.waitForURL((target) => target.pathname === "/warehouse/inventory", { timeout: 30000 }),
    ...
    try {
      ...
      await page.locator("[data-inventory-quick-edit]").first().click();
      await page.locator("[data-inventory-quick-edit-form]").waitFor({ timeout: 15000 });
    } catch (error) {
      throw new Error(`Wix-style warehouse inventory controls did not render after login: ${JSON.stringify(await routeDiagnostic("warehouse_inventory"))}; ${error instanceof Error ? error.message : String(error)}`);
    }
    const warehouseUrl = page.url();

    await page.goto(`${baseUrl}/warehouse/picking`, { waitUntil: "networkidle" });
    try {
      await page.locator("[data-picking-queue]").waitFor({ timeout: 15000 });
    } catch (error) {
      throw new Error(`Warehouse picking controls did not render: ${JSON.stringify(await routeDiagnostic("warehouse_picking"))}; ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    return {
      ...
      loginPath: "/login?next=/warehouse/inventory",
      warehouseInventoryUrl: warehouseUrl,
      warehouseForms: "VERIFIED",
      pickingQueue: "VERIFIED",
