...
export async function GET(_request: Request, context: RouteContext) {
  ...
  const [paymentsResponse, activityResponse] = await Promise.all([
    fetch(
    ...
    fetch(
      `${config.url}/rest/v1/activity_logs?select=id,action,severity,metadata,created_at&entity_table=eq.orders&entity_id=eq.${encodeURIComponent(orderId)}&order=created_at.desc&limit=20`,
      { headers, cache: "no-store" }
    )
  ]);
  ...
  return NextResponse.json({ payments, activity });
}
