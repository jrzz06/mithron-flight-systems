...
export function cleanupEditorHtmlMarkup(html: string) {
  ...
  for (let i = 0; i < 8; i += 1) {
    const next = value.replace(
      /<span(?=\s|>)(?![^>]*\bclass=)(?![^>]*\bdata-)[^>]*>([\s\S]*?)<\/span>/gi,
      "$1"
    );
    if (next === value) break;
    value = next;
 ...
 * Strips inline styles, sanitizes, and decodes escaped markup.
 */
export function prepareEditorHtmlForDisplay(raw: string | null | undefined) {
  ...
  return cleanupEditorHtmlMarkup(sanitizeEditorHtml(html));
}

export const prepareEditorHtmlForSave = prepareEditorHtmlForDisplay;
