...
export function isWarehouseEligible(order: OrderLike) {
  ...
  const allowedPayment = paymentStatus === "succeeded" || paymentStatus === "not_required";
  const allowedStatus = ["assigned", "processing", "packed", "dispatched", "in_transit", "delivered"].includes(status);

  return allowedPayment && allowedStatus;
}
...
export function paymentAllowsFulfillment(paymentStatus: unknown): paymentStatus is PaymentStatus {
