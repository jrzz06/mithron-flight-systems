...
export function buildOptimisticCartLines(items: PersistedCartItem[]): CartItem[] {
  return items.map((item) => ({
    ...
    bundleName: item.bundleName?.trim() || "Standard configuration",
    unitPrice: 0,
    compareAt: null,
...
export function cartLinesMatchPersisted(persisted: PersistedCartItem[], resolved: CartItem[]) {
  ...
  const resolvedByKey = new Map(
    resolved.map((line) => [`${line.productSlug}:${line.bundleId}`, line.quantity] as const)
  );

  return persisted.every(
    (item) => resolvedByKey.get(`${item.productSlug}:${item.bundleId}`) === item.quantity
  );
}
