...
const USE_CASE_COPY: Record<string, { description: string; benefits: string[] }> = {
  ...
  mapping: {
    ...
    benefits: ["Large-area coverage", "GIS-ready outputs", "Efficient revisit workflows"]
  },
  inspection: {
  ...
  industrial: {
    ...
    benefits: ["Rugged deployment", "Operator-ready workflows", "Serviceable platform"]
  }
};
...
export function buildProductMediaPlan(product: Product): ProductMediaPlanItem[] {
  const pool = sortMediaAssets(uniqueMediaBySrc([product.hero, product.image, ...product.gallery]));
...
export function buildProductUseCases(product: Product): ProductUseCase[] {
  ...
  const cases = interests.map((interest) => {
    ...
    const preset = USE_CASE_COPY[key] ?? USE_CASE_COPY[interest.toLowerCase()] ?? {
      ...
      benefits: ["Mission-ready configuration", "Operator support", "Platform compatibility"]
    };
    return {
      id: key,
