...
function splitMenuColumns(products: Product[], categoryHref: string) {
  ...
  if (!columnOne.length) {
    return {
      columnOne: [{ label: "View all systems", href: categoryHref, featureKey: "view-all" }],
      columnTwo: []
    };
  }
  ...
  if (!columnTwo.length) {
    return {
      columnOne,
      columnTwo: [{ label: "View all systems", href: categoryHref, featureKey: "view-all" }]
    };
  }
...
export function buildEnterpriseMenuConfigs(products: Product[]): EnterpriseMenuConfig[] {
  return catalogCategoryDefinitions.map((definition) => {
    ...
    if (definition.menuType === "compact") {
      return buildCompactMenu(definition, categoryProducts);
    }

    if (definition.menuType === "franchise") {
      return buildFranchiseMenu(definition, categoryProducts);
    }
