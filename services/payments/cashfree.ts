...
export class CashfreeGateway implements PaymentGateway {
  ...
  async fetchPaymentStatus(intentId: string): Promise<PaymentEvent> {
    const response = await fetch(`${cashfreeApiBase(this.env)}/orders/${encodeURIComponent(intentId)}`, {
      method: "GET",
      headers: cashfreeHeaders(this.env),
      cache: "no-store"
    });
  ...
  async refund(intentId: string, amount?: number): Promise<RefundResult> {
    const response = await fetch(`${cashfreeApiBase(this.env)}/orders/${encodeURIComponent(intentId)}/refunds`, {
      method: "POST",
      headers: cashfreeHeaders(this.env),
      body: JSON.stringify({
...
export async function safeFetchCashfreePaymentStatus(intentId: string, env: Record<string, string | undefined>) {
  try {
