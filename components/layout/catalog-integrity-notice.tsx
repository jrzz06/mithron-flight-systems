      ...
      {unavailable.length ? (
        ...
        >
          <div className="mx-auto flex w-full max-w-[1740px] flex-col gap-2">
            <p className="text-sm font-semibold tracking-tight">Catalog connection is unstable</p>
            <p className="text-sm text-[#64748b]">
              Mithron could not reach the product catalog reliably. Navigation may be limited until the connection recovers. Refresh the page or try again shortly.
            </p>
...
export function CatalogDataErrorPanel({
  ...
  const resolvedDescription = description ?? (
    error.code === "catalog_unavailable"
      ? "Mithron could not reach the product catalog right now. Refresh the page or try again in a moment."
      : "The product is published, but Mithron could not resolve its source image. Fix the catalog media link or unpublish the listing."
  );
