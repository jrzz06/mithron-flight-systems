...
describe("warehouse packing workflow", () => {
  ...
  it("wires picking, packing, and dispatch pages to functional warehouse workflow components", () => {
    const picking = readFileSync(join(process.cwd(), "app/warehouse/picking/page.tsx"), "utf8");
    const packing = readFileSync(join(process.cwd(), "app/warehouse/packing/page.tsx"), "utf8");
    const dispatch = readFileSync(join(process.cwd(), "app/warehouse/dispatch/page.tsx"), "utf8");
    const packingCard = readFileSync(join(process.cwd(), "components/warehouse/warehouse-packing-order-card.tsx"), "utf8");
    ...
    expect(picking).toContain("WarehousePickingTable");
    expect(picking).toContain("data-barcode-ready");
    expect(packing).toContain("WarehousePackingOrderCard");
    expect(packing).toContain("completeWarehousePackingFormAction");
    expect(packingCard).toContain("data-packing-checklist");
    const settings = readFileSync(join(process.cwd(), "app/warehouse/settings/page.tsx"), "utf8");
    expect(dispatch).toContain('["packed", "ready_for_pickup"]');
    expect(dispatch).toContain("itemsByShipment");
    expect(settings).toContain("WarehouseStationSettingsForm");
