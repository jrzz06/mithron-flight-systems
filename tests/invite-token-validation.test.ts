...
describe("invite token validation", () => {
  ...
  it("wires invite resolution into auth callback and provision routes", () => {
    const callback = readFileSync(join(process.cwd(), "app/auth/callback/route.ts"), "utf8");
    const provision = readFileSync(join(process.cwd(), "app/api/auth/provision/route.ts"), "utf8");
    expect(callback).toContain("resolveInviteRoleForUser");
    expect(callback).not.toContain("app_metadata?.role");
    expect(provision).toContain("resolveInviteRoleForUser");
    expect(provision).toContain("Provisioning failed.");
  });
});
