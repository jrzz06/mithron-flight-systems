...
describe("shipment persistence and fulfillment lifecycle", () => {
  ...
  it("derives order fulfillment status from shipment coverage and statuses", () => {
    ...
    expect(deriveOrderFulfillmentStatusFromShipments([{ id: "item-1", quantity: 2 }], [{ order_item_id: "item-1", quantity: 2 }], [{ shipment_status: "damaged" }])).toBe("packed");
  });
  ...
  it("wires shipment persistence through admin mutations, warehouse actions, UI, and remote verifier", async () => {
    ...
    const dispatchPage = readFileSync(join(process.cwd(), "app", "warehouse", "dispatch", "page.tsx"), "utf8");
    const shipmentsPage = readFileSync(join(process.cwd(), "app", "warehouse", "shipments", "page.tsx"), "utf8");
    const shipmentDetailPage = readFileSync(join(process.cwd(), "app", "warehouse", "shipments", "[id]", "page.tsx"), "utf8");
    ...
    expect(shipmentsPage).toContain("damaged");
    expect(shipmentDetailPage).toContain("data-shipment-timeline");
    expect(shipmentDetailPage).toContain("value=\"reserved\"");
    expect(shipmentDetailPage).toContain("value=\"damaged\"");
