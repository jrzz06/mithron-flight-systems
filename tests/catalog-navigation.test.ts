...
describe("catalog navigation menus", () => {
  it("builds mega menus from live catalog products instead of static arrays", async () => {
    ...
    expect(globalMenu?.type).toBe("franchise");
    if (globalMenu?.type === "franchise") {
      expect(globalMenu.href).toBe("/category/global-products");
      expect(globalMenu.items.some((item) => item.href.startsWith("/product/"))).toBe(true);
      expect(globalMenu.card.href.startsWith("/product/")).toBe(true);
    }
  });
});
