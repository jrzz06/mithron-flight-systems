...
describe("catalog search fallback and resilience", () => {
  ...
  it("prefetches the search index in the overlay and nav", () => {
    ...
    expect(searchOverlay).toContain("searchCatalogIndex");
    expect(storeNav).toContain("intent=index");
  });
  ...
  it("points SearchAction to the search page", () => {
    const structuredData = source("lib/structured-data.ts");

    expect(structuredData).toContain('toAbsoluteUrl("/search")}?q={search_term_string}');
