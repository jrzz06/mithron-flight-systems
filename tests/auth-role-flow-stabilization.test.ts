...
describe("phase 1 auth and role flow stabilization", () => {
  ...
  it("keeps role-specific protected routes isolated", () => {
    ...
    expect(canAccessProtectedPath("warehouse", "/warehouse/inventory")).toBe(true);
    expect(canAccessProtectedPath("warehouse", "/warehouse/shipments")).toBe(true);
  ...
  it("redirects control panel roles away from storefront login targets", () => {
    ...
    expect(getRoleAwareAuthRedirectPath("/warehouse/inventory", "warehouse")).toBe("/warehouse/inventory");
  ...
  it("labels staff workspaces separately from the customer hub", () => {
    expect(workspaceLabelForRole("admin")).toBe("Admin workspace");
    expect(workspaceLabelForRole("supplier")).toBe("Supplier portal");
    expect(workspaceLabelForRole("warehouse")).toBe("Warehouse operations");
